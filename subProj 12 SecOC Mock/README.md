...
* **`secoc_crypto.py`**: A python library simulating the core AES-CMAC logic (via `hashlib`/`hmac`). It mathematically hashes the target payload alongside an internal monotonic counter (FV) and the CAN Message ID, explicitly asserting algorithms to trap and reject stale freshness values, mismatched network IDs, or manipulated byte arrays.
* **`secoc_nodes.py`**: Simulates the endpoints of a CAN network:
  * `TransmitterECU`: Generates the MAC (with configurable truncation lengths), synchronizes the FV, and constructs the secure dictionary simulating a SecOC-compliant CAN payload.
...
* **TC-1204 Wrong Data ID Rejection**: Asserts that migrating a perfectly valid `Payload` + `MAC` packet to a different CAN Message ID completely fails validation, proving the SecOC MAC is strictly bound to its specific network context.
* **TC-1205 Truncated MAC Validation**: Asserts the cryptography successfully hashes and verifies when configured for arbitrary target truncation profiles (e.g., matching a 2-byte hash versus a 4-byte hash), explicitly rejecting spoofed lengths.
